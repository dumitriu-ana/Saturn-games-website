<?php
  ...
  if(isset($_FILES['uploaded-file'])){
    ...
    $path = "../assets/fonts/".$uniq_id.".".strtolower($ext);
    ...
    try{
      ...
      echo "<script>
        if(parent.error_upload_font){
          parent.error_upload_font('".$e->getMessage()."');
        }
      </script>";
    }
  }
      ...
      <form method="post" enctype="multipart/form-data" id="upload-form" action="<?php echo $_SERVER['PHP_SELF']; ?>">
        <div class="file">
          <input type="file" name="uploaded-file" class="filestyle" data-classButton="btn btn-success" data-input="false" data-classIcon="icon-plus" data-buttonText="Your label here."
            id="upload-asset" accept=".ttf,.otf,.woff"
            onchange="
              if(parent.startLoadingFonts){
                parent.startLoadingFonts();
              }
              $('#upload-form').submit();
            "
          >
