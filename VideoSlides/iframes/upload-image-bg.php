<?php
  ...
  if(isset($_FILES['uploaded-file'])){
    ...
    try{
      ...
      echo "<script>
        if(parent.error_bg_upload){
          parent.error_bg_upload('".$e->getMessage()."');
        }
      </script>";
      $uploaded = 0;
    }
...
<html lang="en" dir="ltr">
  ...
  <body>
    <center>
      <form method="post" enctype="multipart/form-data" id="upload-bg-form" action="<?php echo $_SERVER['PHP_SELF']; ?>">
        <div class="file">
          <input type="file" name="uploaded-file" class="filestyle" data-classButton="btn btn-success" data-input="false" data-classIcon="icon-plus" data-buttonText="Your label here."
            id="upload-asset-bg" accept="image/*"
            onchange="
              if(parent.startLoadingBG){
                parent.startLoadingBG();
              }
              $('#upload-bg-form').submit();
            "
          >
        </div>
      </form>
